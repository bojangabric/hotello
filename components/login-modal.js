const LoginModal = ({ show, toggle }) => (
  <div
    className={`${
      show ? 'table' : 'hidden'
    } modal-mask fixed top-0 left-0 w-full h-full min-h-screen text-gray-700 z-50`}
  >
    <div className="table-cell align-middle">
      <div className="modal-container py-5 px-6 lg:rounded w-full h-full md:h-auto overflow-x-hidden overflow-y-visible bg-white mx-auto">
        <div className="flex flex-col flex-1 justify-center relative">
          <button onClick={toggle} className="absolute top-0 right-0 focus:outline-none">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="fill-current w-8 h-8">
              <path d="M15.78 14.36a1 1 0 01-1.42 1.42l-2.82-2.83-2.83 2.83a1 1 0 11-1.42-1.42l2.83-2.82L7.3 8.7a1 1 0 011.42-1.42l2.83 2.83 2.82-2.83a1 1 0 011.42 1.42l-2.83 2.83 2.83 2.82z" />
            </svg>
          </button>
          <div className="w-full mt-4">
            <form className="mx-auto" method="POST">
              ...
              <div className="flex sm:block sm:text-center md:flex flex-col mt-6">
                <button
                  type="submit"
                  className="bg-blue-500 hover:bg-blue-700 text-white text-sm font-semibold py-2 px-20 rounded"
                >
                  Login
                </button>
              </div>
          ...
          <div
            className="text-center mx-auto text-sm text-gray-500 my-10 w-full sm:w-4/5 md:w-full border-b border-gray-400"
